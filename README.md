...
* **Field observability:** Opt-in `--observe-fields` mode surfaces which event fields no rule references (gap signal) and which rule fields have never appeared in an event (broken-coverage signal) over `GET /api/v1/fields*`, with Prometheus counters and a bounded in-memory observer
