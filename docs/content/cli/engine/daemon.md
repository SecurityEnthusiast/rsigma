...
Each sink serializes in its own wire format, selected with `?format=`. rsigma's native NDJSON is the default and the only format. The parameter is accepted on `stdout`, `file://`, `nats://`, and `unix://` sinks only, and `format=` on an OTLP spec, the `--dlq`, or the audit sink fails startup, as does an unknown value.
