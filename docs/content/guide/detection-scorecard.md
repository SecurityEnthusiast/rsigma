...
- **keep**: the rule is precise enough (precision proxy at or above the keep floor, default `0.80`), carries enough volume, and, when last-fired is known via `--metrics-window`, has fired recently enough. Leave it alone.
...
- [Triage Feedback Loop](triage-feedback.md) for the disposition feed `--triage` consumes.
- [CI/CD](ci-cd.md) for wiring the scorecard as the metrics gate.
