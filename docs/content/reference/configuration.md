...
| `daemon.api.audit` | `engine daemon` | Control-plane mutation audit trail (`enabled`, `max_entries`, `max_age`, `max_body_bytes`, `sink`). Requires `--state-db`; auto-enabled when a state database is configured. See [HTTP API: Audit trail](http-api.md#audit-trail). |
...
| `daemon.capture` | `engine daemon` | Verdict-driven evidence capture (`enabled`, `spool_dir`, ring and disk bounds, `ttl`, `spool_queue_capacity`). Disabled by default and startup-only; enable with `enabled: true` or `--enable-capture`. Requires dispositions, `group_by`, and `spool_dir`. See [Verdict-Driven Corpora](../guide/verdict-to-corpus.md). |
