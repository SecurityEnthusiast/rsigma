...
| `RSIGMA_MCP_DAEMON_TOKEN` | string | unset | `mcp serve --daemon-url` | Bearer token the MCP server sends to the daemon (`Authorization: Bearer <token>`). Equivalent to `--daemon-token`. Secret: flag/env only, never read from config files. |
