...
| `rstix` | STIX 2.1 + TAXII 2.1 library crate. **Data Model + Serialization** complete (`serde`, default). **Pattern Engine** complete (`pattern`). | `Bundle::parse` / `parse_reader`, `Bundle::validate`, `ParseOptions` + typed `TypeRegistry`, 42 typed object families (`serde`); `Pattern::parse` / `evaluate` / `matches_single` / `matches_single_with_bundle` / `evaluate_observed_data` / `canonical`, `IndicatorPattern` STIX AST wiring (`pattern` + `serde`); `model::validate` invariants, deterministic SCO IDs, vocabulary tables | `serde` (default), `pattern` |
