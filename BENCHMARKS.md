...
**Fixtures**: `scripts/perf/fetch-fixtures.sh` (pinned corpus + deterministic event lanes), run via `scripts/perf/baseline-eval.sh` and `scripts/perf/daemon-matrix.sh`, both against the same two binaries via their `RSIGMA` override. The offline harness now reports the median of three measured runs; the daemon matrix also covers `--include-event` and a handcrafted correlation lane. `scripts/perf/image-compare.sh` races two container images over the same fixtures instead of two native binaries, which is how cost that only the released artifact carries becomes visible; it needs Docker but no Rust toolchain and no local k6.
...
The formatted daemon path now parses lines in parallel before taking the engine mutex, while a separate ordering lock keeps one batch in flight and preserves correlation and output order. On the raw Windows lane with `--logsource-routing`, `--batch-size 512`, 16 k6 VUs, and the same pinned corpus, a same-build median-of-three comparison moved 8-thread throughput from 306,351 to 384,906 events/s while one-thread throughput stayed near 90,000 events/s. Scaling improved from 3.39x (42% efficiency) to 4.19x (52% efficiency); parse share at eight threads fell from about 40% to 27%. Repeated runs after sustained compilation and benchmarking measured 50-52% efficiency, so parsing was confirmed as a major bottleneck but not the only serial cost. A `Bytes`/request-body-span prototype did not improve this workload and was not retained; the daemon does retain single-parse DLQ routing, eliminating its previous validation parse.

Phase timing after that change showed `evaluate` still ~71% of measured batch time at eight threads. Sampling profiles attributed most of that to candidate-index field probes and `JsonEvent::get_field` misses over absent Sigma fields. Probing only witnesses under the event's top-level keys, plus a dotted-path root fast-miss in `get_field`, raised the same routed daemon median from about 93k to 102k events/s at one thread and from about 386k to 404k events/s at eight threads on the same host.

Those ratios hold only when the allocator scales with the fan-out, which is a property of the build rather than of the engine. Every figure above is a native build against the system allocator, while the released `ghcr.io/timescale/rsigma` binary is static musl, and musl's mallocng serializes the concurrent allocation `evaluate_batch` performs. Until jemalloc became the musl global allocator the released artifact therefore did not scale at all. Measured on an AMD Ryzen 7 PRO 6850U (8 cores / 16 threads, Linux) over the pinned corpus and the raw-blob lane, with the daemon pinned to six physical cores and the load generator to the other two, `--batch-size 512` and a `/dev/null` sink; the daemon column is the median of three 30 s runs from `scripts/perf/image-compare.sh` and the single-core column comes from `scripts/perf/baseline-eval.sh` on the same two binaries:

| Variant | Build | Single core | Daemon, 6 cores | Scaling |
|---------|-------|------------:|----------------:|--------:|
| default | musl | 26,557 | 36,875 | 1.39x |
| | musl + jemalloc | 30,045 | 95,505 | 3.18x |
| `--logsource-routing` | musl | 57,463 | 53,435 | 0.93x |
| | musl + jemalloc | 65,826 | 160,238 | 2.44x |

Stock musl scales at **0.93x** with routing: six cores deliver less than one, so the fan-out costs more than it returns. Changing allocator is worth ~14% single-threaded and 2.6-3.0x on the daemon, and that disproportion is the signature of allocator contention rather than per-event cost. Match counts are identical in every cell. jemalloc also reaches these figures on *fewer* cores (6.7-6.8 against 8.0), and halves rule load from 0.86 s to 0.49 s. Because none of this is visible to a native build, it needs `scripts/perf/image-compare.sh` rather than the harnesses above.

The witness audit (`cargo run --release -p rsigma-eval --example witness_audit`) shows 99.6% of the corpus carries a sound required-positive witness. The production `CandidateIndex`, measured directly over 10,000 events in each deterministic lane, returns a p95 of 0.29-3.86% of the 3,132 loaded rules: 0.38% on raw Windows and Cisco syslog, 3.74% on structured Windows, and 3.19% on Sysmon file events. This confirms the below-10% criterion with the actual index rather than the audit's intentionally over-approximating simulation. Peak RSS for load + eval is ~113 MB. Sampling profiles taken before witness indexing attributed per-event time to event field lookup and hashing (~25-45%), `std` substring search and searcher setup (~13-30%), the condition and detection tree walk (~15%), case folding, and allocator traffic. Witness indexing removed most of that work rather than making it cheaper, so the remaining profile is due a re-capture before any further micro-optimization.
...
The daemon matrix measures `--include-event` both with and without routing, including the match-heavy lane, because cloning the complete input into each result is material at ~3 matches per event and nearly free at ~0.02. Its checked-in correlation lane exercises event count, value count, value sum, and ordered temporal processing instead of relying on SigmaHQ, because the pinned corpus contains no correlation rules.
