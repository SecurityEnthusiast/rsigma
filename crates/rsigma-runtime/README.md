...
- **I/O**: `EventSource` trait (stdin, HTTP, NATS, Unix domain socket) and `Sink` enum (stdout, file, NATS, OTLP, webhook, Unix domain socket) with fan-out support and a shared async delivery layer (per-sink queues, bounded retry/backoff, DLQ routing). The `unix://` source and sink are gated behind the `uds` feature and `#[cfg(unix)]`.
