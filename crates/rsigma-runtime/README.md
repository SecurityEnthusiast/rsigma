...
- **`LogProcessor`**: ordered batch evaluation pipeline with parallel input parsing, atomic engine swap via `ArcSwap`, `MetricsHook` for pluggable metrics, and `EventFilter` for JSON payload extraction. `process_batch_with_format_detailed` returns a `BatchProcessOutcome` whose `parse_error_indices` lets adapters route malformed lines without parsing them twice.
...
- **I/O**: `EventSource` trait (stdin, HTTP, NATS, Unix domain socket) and `Sink` enum (stdout, file, NATS, OTLP, webhook, Unix domain socket) with fan-out support and a shared async delivery layer (per-sink queues, bounded retry/backoff, DLQ routing). Each line-oriented sink carries a `SinkFormat` (`Ndjson` by default, `Ocsf` for OCSF Detection Finding output). The `unix://` source and sink are gated behind the `uds` feature and `#[cfg(unix)]`.
- **OCSF findings**: the `ocsf` module serializes detections, correlations, incidents, and risk incidents as OCSF Detection Finding (class 2004) JSON, selected per sink with `?format=ocsf`. Output only: everything without a faithful OCSF home rides under `unmapped`, and nothing here deserializes OCSF.
...
- **Alert pipeline**: the `alert_pipeline` module is a post-engine stage that silences, inhibits, and deduplicates results before grouping survivors into incidents, modeled on Alertmanager. It shares the field-selector namespace and scope filter with the risk layer, emits `IncidentResult`s via `Sink::send_incident`, and keeps a versioned snapshot (active alerts, open incidents, dynamic silences, inhibition sources) for persistence.
- **Risk-based alerting**: the `risk` module (`RiskLayer`, `RiskState`, `RiskIncidentResult`, `parse_risk_config`, `build_risk_layer`) annotates each firing with a risk score and risk objects (entities) via the shared field selector, accumulates per-entity risk over a sliding window, and emits a `RiskIncidentResult` on a score or ATT&CK-tactic-count threshold, with caps, a per-entity cooldown, and a versioned snapshot for persistence. It runs after enrichment and before the alert pipeline.
...
| `uds` | Enable the Unix domain socket event source and sink (`unix://` input/output). Unix targets only |
