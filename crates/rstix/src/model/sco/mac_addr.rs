//! STIX `mac-addr` objects (STIX §6.10).
...
#[cfg(feature = "serde")]
fn deserialize_mac_addr_type<'de, D>(deserializer: D) -> Result<String, D::Error>
where
    D: serde::Deserializer<'de>,
{
    crate::model::type_check::deserialize_stix_type_field(deserializer, MacAddr::TYPE_NAME)
}
...
impl QueryableStixObject for MacAddr {
    fn id(&self) -> &StixId {
        &self.common.id
    }
...
mod tests {
    ...
    fn rejects_wrong_type_field() {
        ...
        let msg = serde_json::from_str::<MacAddr>(json)
            .unwrap_err()
            .to_string();
        assert!(msg.contains("expected STIX type `mac-addr`"));
        assert!(msg.contains("got `"));
    }

    #[test]
    fn rejects_missing_type_field() {
        let json = include_str!("../../../tests/fixtures/spec/sco/mac-addr.json");
        let value: serde_json::Value = serde_json::from_str(json).expect("json");
        let mut obj = value.as_object().expect("object").clone();
        obj.remove("type");
        let err = serde_json::from_value::<MacAddr>(serde_json::Value::Object(obj)).unwrap_err();
