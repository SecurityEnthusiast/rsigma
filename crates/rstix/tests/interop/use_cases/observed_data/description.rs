...
pub fn assert_description_scope() {
    ...
    let create_bundle = validate_interop_fixture(FIXTURE_CREATE, &create.json)
        .expect("§3.14.3.1 must parse");
    let od_id = StixId::parse("observed-data--cf8eaa41-6f4c-482e-89b9-9cd2d6a83cb1").expect("id");
    let od = create_bundle.get_typed::<ObservedData>(&od_id).expect("typed ObservedData");
...
interop_test!("REQ-3.14-1", "use_cases::observed_data::description::description_scope", description_scope, { assert_description_scope(); });
