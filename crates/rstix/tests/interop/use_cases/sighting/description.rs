...
pub fn assert_description_scope() {
    let bundle = validate_interop_fixture(FIXTURE_CREATE, &load_fixture(FIXTURE_CREATE).json).unwrap();
...
interop_test!("REQ-3.17-1", "use_cases::sighting::description::description_scope", description_scope, { assert_description_scope(); });
