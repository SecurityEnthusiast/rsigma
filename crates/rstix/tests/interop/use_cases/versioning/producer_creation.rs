...
pub fn assert_select_content() {
    ...
    let use_case_ids = use_case_object_ids(FIXTURE_CREATE_INDICATOR, &parse_fixture_objects(&json).unwrap());
    let bundle = validate_interop_json(&json, &InteropGateOptions { use_case_object_ids: use_case_ids.clone() }).unwrap();
    let stix_id = StixId::parse(&use_case_ids[0]).unwrap();
    let indicator = bundle.get_typed::<Indicator>(&stix_id).unwrap();
    assert_eq!(indicator.name.as_deref(), Some("Caller-selected versioning Indicator"));
}
...
pub fn assert_create_indicator() {
    validate_interop_fixture(FIXTURE_CREATE_INDICATOR, &load_fixture(FIXTURE_CREATE_INDICATOR).json).unwrap();
}

pub fn assert_create_sighting() {
    validate_interop_fixture(FIXTURE_CREATE_SIGHTING, &load_fixture(FIXTURE_CREATE_SIGHTING).json).unwrap();
}
...
interop_test!("REQ-3.20-P-C-01", "use_cases::versioning::producer_creation::select_content", select_content, { assert_select_content(); });
interop_test!("REQ-3.20-P-C-02", "use_cases::versioning::producer_creation::identity_compliance", identity_compliance, { assert_identity_compliance(); });
interop_test!("REQ-3.20-P-C-03", "use_cases::versioning::producer_creation::create_indicator", create_indicator, { assert_create_indicator(); });
interop_test!("REQ-3.20-P-C-04", "use_cases::versioning::producer_creation::create_sighting", create_sighting, { assert_create_sighting(); });
interop_test!("REQ-CHK-SXP-3.20-C", "use_cases::versioning::producer_creation::producer_testcase_data", producer_testcase_data, { assert_producer_testcase_data(); });
