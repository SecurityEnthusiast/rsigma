...
pub fn assert_select_content() {
    let fixture = load_fixture(FIXTURE_REV_INDICATOR);
    ...
    for object in objects.iter_mut() {
        if object.get("type").and_then(Value::as_str) == Some("indicator") {
            object["name"] = Value::String("Caller-selected revoked Indicator".into());
            renamed += 1;
        }
    }
    assert_eq!(
    ...
    .expect("caller-selected revoked bundle must pass interop gate");
    assert_eq!(
        use_case_ids.len(),
        1,
    ...
    assert_eq!(
        ...
        Some("Caller-selected revoked Indicator"),
        "caller-selected name must survive parse and re-validation"
    );
    assert_eq!(
        indicator.common.revoked,
        Some(true),
        "caller-selected revoked Indicator must retain revoked=true"
    );
}
    ...
    for relative in REVOCATION_FIXTURES {
        ...
        for object_id in use_case_ids {
            let wire = objects
                ...
                .unwrap();
            assert_eq!(
                wire.get("revoked").and_then(Value::as_bool),
                Some(true),
                "{relative}: revoked must be true"
            );
        }
    }
}
...
pub fn assert_no_new_version_of_revoked_id() {
    for relative in REVOCATION_FIXTURES {
        ...
        for object_id in &use_case_ids {
            ...
            assert_eq!(
                same_id[0].get("revoked").and_then(Value::as_bool),
                Some(true)
            );
            ...
            assert!(
                !non_revoked_same,
                "{relative}: no non-revoked sibling for revoked id"
            );
        }
    }
}
...
interop_test!(
    "REQ-3.20-P-R-01",
    "use_cases::versioning::producer_revocation::select_content",
    select_content,
    {
...
interop_test!(
    "REQ-3.20-P-R-02",
    "use_cases::versioning::producer_revocation::identity_compliance",
    identity_compliance,
    {
...
interop_test!(
    "REQ-3.20-P-R-03",
    "use_cases::versioning::producer_revocation::revoked_true",
    revoked_true,
    {
        assert_revoked_true();
    }
...
interop_test!(
    "REQ-3.20-P-R-04",
    "use_cases::versioning::producer_revocation::modified_after_created",
...
interop_test!(
    "REQ-3.20-P-R-05",
    "use_cases::versioning::producer_revocation::created_preserved",
    created_preserved,
    {
...
interop_test!(
    "REQ-3.20-REV-X-01",
    "use_cases::versioning::producer_revocation::no_new_version_of_revoked_id",
    no_new_version_of_revoked_id,
    {
        assert_no_new_version_of_revoked_id();
    }
);
interop_test!(
    "REQ-CHK-SXP-3.20-R",
    "use_cases::versioning::producer_revocation::producer_testcase_data",
