//! §3.9.5 Required Consumer Persona Support (REQ-3.9-C-01..C-05).

use crate::interop_test;
use rstix::core::{QueryValue, QueryableStixObject, StixId};
use rstix::model::sdo::{Identity, IntrusionSet};
...
/// REQ-3.9-C-01 — Consumer supports §3.9.2 Producer Persona properties on normative fixtures.
    ...
    for_each_fixture(|relative| {
        ...
        for object_id in use_case_ids {
            ...
            assert_eq!(
                ...
                "{relative}: resource_level must match wire"
            );
            assert_eq!(
                intrusion_set.primary_motivation.as_deref(),
                wire.get("primary_motivation").and_then(Value::as_str),
                "{relative}: primary_motivation must match wire"
            );
...
/// REQ-3.9-C-02 — Consumer receives Identity and Intrusion Set(s) (§3.9.3.1).
pub fn assert_receives_triad() {
    ...
    assert_eq!(summary.primary_sdo_count, 1, "single Intrusion Set SDO");
...
/// REQ-3.9-C-03 — Consumer resolves `created_by_ref` Identity fields (§3.9.3.1).
...
/// REQ-3.9-C-04 — Consumer processes Intrusion Set fields via query + typed validate.
///
/// Distinct from C-01 (wire re-serialize preservation of Table 18 props): uses
/// [`QueryableStixObject::get_field`] for `name` / `created_by_ref` and runs
/// [`IntrusionSet::validate`], matching query results to the wire object.
pub fn assert_processes_fields() {
    ...
    assert_eq!(
        use_case_ids.len(),
        1,
        "{relative}: one intrusion-set use-case id"
    );
    ...
    let wire = objects
        .iter()
        .find(|obj| obj.get("id").and_then(Value::as_str) == Some(object_id.as_str()))
        .unwrap_or_else(|| panic!("{relative}: wire intrusion-set {object_id}"));
    let intrusion_set = bundle
        .get_typed::<IntrusionSet>(&stix_id)
        .unwrap_or_else(|| panic!("{relative}: typed intrusion-set {object_id}"));
    ...
    intrusion_set
        .validate()
        .unwrap_or_else(|err| panic!("{relative}: IntrusionSet::validate: {err}"));

    match intrusion_set.get_field(&["name"]) {
        Some(QueryValue::Str(name)) => {
            assert_eq!(
                name,
                intrusion_set.name.as_str(),
    ...
    match intrusion_set.get_field(&["created_by_ref"]) {
...
/// REQ-3.9-C-05 — Consumer processes related SDOs/SROs when present (§3.9.3.1 has none).
///
/// Distinct from description scope: asserts the consumer path when no SROs are bundled —
/// relationship count is zero and the Intrusion Set name still resolves.
pub fn assert_processes_related() {
    ...
    let intrusion_set = bundle
        .objects_of_type::<IntrusionSet>()
        .next()
        .expect("intrusion-set");
    assert_eq!(intrusion_set.name, "Bobcat Breakin");
}

/// REQ-CHK-SXC-3.9 / §4.2 Table 55 — Consumer handles §3.9.3 Producer test case data.
pub fn assert_handles_producer_testcases() {
    for relative in PRODUCER_FIXTURES {
        ...
        let bundle = validate_interop_fixture(relative, &fixture.json).unwrap_or_else(|err| {
            panic!("{relative}: §3.9 consumer must handle producer test case: {err}")
        });

        assert!(
            !use_case_ids.is_empty(),
            "{relative}: expected intrusion-set use-case object(s)"
        );
        for object_id in use_case_ids {
            let stix_id = StixId::parse(&object_id).expect("intrusion-set id");
            let intrusion_set = bundle
...
interop_test!(
    "REQ-3.9-C-01",
    "use_cases::intrusion_set::consumer::supports_producer_props",
...
interop_test!(
    "REQ-3.9-C-02",
    "use_cases::intrusion_set::consumer::receives_triad",
    receives_triad,
    {
...
interop_test!(
    "REQ-3.9-C-03",
    "use_cases::intrusion_set::consumer::resolves_created_by_ref",
...
interop_test!(
    "REQ-3.9-C-04",
    "use_cases::intrusion_set::consumer::processes_fields",
    processes_fields,
    {
...
interop_test!(
    "REQ-3.9-C-05",
    "use_cases::intrusion_set::consumer::processes_related",
    processes_related,
    {
...
interop_test!(
    "REQ-CHK-SXC-3.9",
    "use_cases::intrusion_set::consumer::handles_producer_testcases",
