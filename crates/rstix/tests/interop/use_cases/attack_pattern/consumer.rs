...
/// member. Pins `len() == 1` so those leaf loops are non-vacuous on this fixture.
pub fn assert_processes_fields() {
    ...
    assert_eq!(
        ...
        1,
        "{relative}: one kill_chain_phase for leaf processing"
    );
    ...
    assert_eq!(
        ...
        1,
        "{relative}: one external_reference for leaf processing"
    );
