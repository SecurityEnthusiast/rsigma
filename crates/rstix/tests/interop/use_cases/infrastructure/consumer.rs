//! §3.8.5 Required Consumer Persona Support (REQ-3.8-C-01..C-05).

use crate::interop_test;
use rstix::core::{QueryValue, QueryableStixObject, StixId};
use rstix::model::sdo::{Identity, Infrastructure};
...
/// REQ-3.8-C-01 — Consumer supports §3.8.2 Producer Persona properties on normative fixtures.
...
/// REQ-3.8-C-02 — Consumer receives Identity and Infrastructure(s) (§3.8.3.1).
pub fn assert_receives_triad() {
    ...
    assert_eq!(summary.primary_sdo_count, 1, "single Infrastructure SDO");
...
/// REQ-3.8-C-03 — Consumer resolves `created_by_ref` Identity fields (§3.8.3.1).
...
/// REQ-3.8-C-04 — Consumer processes Infrastructure fields via query + typed validate.
///
/// Distinct from C-01 (wire re-serialize preservation of Table 16 props): uses
/// [`QueryableStixObject::get_field`] for `name` / `created_by_ref` and runs
/// [`Infrastructure::validate`], matching query results and `infrastructure_types`
/// to the wire object.
pub fn assert_processes_fields() {
    ...
    assert_eq!(
        use_case_ids.len(),
        1,
        "{relative}: one infrastructure use-case id"
    );
    ...
    let wire = objects
        .iter()
        .find(|obj| obj.get("id").and_then(Value::as_str) == Some(object_id.as_str()))
        .unwrap_or_else(|| panic!("{relative}: wire infrastructure {object_id}"));
    let infrastructure = bundle
        .get_typed::<Infrastructure>(&stix_id)
        .unwrap_or_else(|| panic!("{relative}: typed infrastructure {object_id}"));
    ...
    infrastructure
        .validate()
        .unwrap_or_else(|err| panic!("{relative}: Infrastructure::validate: {err}"));

    match infrastructure.get_field(&["name"]) {
        Some(QueryValue::Str(name)) => {
            assert_eq!(
                name,
                infrastructure.name.as_str(),
    ...
    match infrastructure.get_field(&["created_by_ref"]) {
    ...
    let wire_types: Vec<_> = wire
        ...
        .expect("wire infrastructure_types")
        .iter()
        .filter_map(Value::as_str)
        .collect();
    assert_eq!(
        infrastructure.infrastructure_types, wire_types,
        "{relative}: typed infrastructure_types must match wire"
    );
}

/// REQ-3.8-C-05 — Consumer processes related SDOs/SROs when present (§3.8.3.1 has none).
///
/// Distinct from description scope: asserts the consumer path when no SROs are bundled —
/// relationship count is zero and the Infrastructure name still resolves.
pub fn assert_processes_related() {
    ...
    let infrastructure = bundle
        .objects_of_type::<Infrastructure>()
        .next()
        .expect("infrastructure");
    assert_eq!(infrastructure.name, "Poison Ivy C2");
}

/// REQ-CHK-SXC-3.8 / §4.2 Table 55 — Consumer handles §3.8.3 Producer test case data.
pub fn assert_handles_producer_testcases() {
    for relative in PRODUCER_FIXTURES {
        ...
        let bundle = validate_interop_fixture(relative, &fixture.json).unwrap_or_else(|err| {
            panic!("{relative}: §3.8 consumer must handle producer test case: {err}")
        });

        assert!(
            !use_case_ids.is_empty(),
            "{relative}: expected infrastructure use-case object(s)"
        );
        for object_id in use_case_ids {
            let stix_id = StixId::parse(&object_id).expect("infrastructure id");
            let infrastructure = bundle
...
interop_test!(
    "REQ-3.8-C-01",
    "use_cases::infrastructure::consumer::supports_producer_props",
...
interop_test!(
    "REQ-3.8-C-02",
    "use_cases::infrastructure::consumer::receives_triad",
    receives_triad,
    {
...
interop_test!(
    "REQ-3.8-C-03",
    "use_cases::infrastructure::consumer::resolves_created_by_ref",
...
interop_test!(
    "REQ-3.8-C-04",
    "use_cases::infrastructure::consumer::processes_fields",
    processes_fields,
    {
...
interop_test!(
    "REQ-3.8-C-05",
    "use_cases::infrastructure::consumer::processes_related",
    processes_related,
    {
...
interop_test!(
    "REQ-CHK-SXC-3.8",
    "use_cases::infrastructure::consumer::handles_producer_testcases",
