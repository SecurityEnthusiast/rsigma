//! §3.5.5 Required Consumer Persona Support (REQ-3.5-C-01..C-05).
...
/// REQ-3.5-C-01 — Consumer supports §3.5.2 Producer Persona properties on normative fixtures.
    ...
    for_each_fixture(|relative| {
        ...
        for object_id in use_case_ids {
            ...
            assert_eq!(
                ...
                1,
                "{relative}: exactly one TLP object_marking_refs entry required"
            );
...
/// REQ-3.5-C-02 — Consumer receives Identity and Indicator(s) with TLP marking (§3.5.3.1).
...
/// REQ-3.5-C-03 — Consumer resolves `created_by_ref` Identity fields (§3.5.3.1).
...
/// REQ-3.5-C-04 — Consumer processes Indicator TLP marking refs against wire (distinct from C-01).
pub fn assert_processes_fields() {
    ...
    let wire_marking_refs = wire
        .get("object_marking_refs")
        .and_then(Value::as_array)
        .expect("wire object_marking_refs");
    assert_eq!(wire_marking_refs.len(), 1);
    assert_eq!(
        indicator.common.object_marking_refs.len(),
        1,
    ...
    match indicator.get_field(&["created_by_ref"]) {
...
/// REQ-3.5-C-05 — Consumer processes related SDOs/SROs when present (§3.5.3.1 has none).
///
/// Distinct from description scope: asserts the consumer path when no SROs are bundled —
/// relationship count is zero and the TLP White marking on the Indicator still resolves.
...
/// REQ-CHK-SXC-3.5 / §4.2 Table 55 — Consumer handles §3.5.3 Producer test case data.
pub fn assert_handles_producer_testcases() {
    for relative in PRODUCER_FIXTURES {
        ...
        let bundle = validate_interop_fixture(relative, &fixture.json).unwrap_or_else(|err| {
            panic!("{relative}: §3.5 consumer must handle producer test case: {err}")
        });
        ...
        for object_id in use_case_ids {
            ...
            assert_eq!(
                indicator.common.object_marking_refs.len(),
                1,
                "{relative}: TLP object_marking_refs must survive consumer close"
            );
...
interop_test!(
    "REQ-3.5-C-01",
    "use_cases::data_markings::consumer::supports_producer_props",
...
interop_test!(
    "REQ-3.5-C-02",
    "use_cases::data_markings::consumer::receives_triad",
    receives_triad,
    {
...
interop_test!(
    "REQ-3.5-C-03",
    "use_cases::data_markings::consumer::resolves_created_by_ref",
...
interop_test!(
    "REQ-3.5-C-04",
    "use_cases::data_markings::consumer::processes_fields",
    processes_fields,
    {
...
interop_test!(
    "REQ-3.5-C-05",
    "use_cases::data_markings::consumer::processes_related",
    processes_related,
    {
...
interop_test!(
    "REQ-CHK-SXC-3.5",
    "use_cases::data_markings::consumer::handles_producer_testcases",
