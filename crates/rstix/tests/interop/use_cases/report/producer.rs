...
fn load_report(relative: &str) -> (Report, String) {
    ...
    assert_eq!(
        use_case_ids.len(),
        1,
        "{relative}: expected one report use-case object"
    );
...
pub fn assert_create_report() {
    validate_interop_fixture(
        FIXTURE_CREATE,
        &working_json(&load_fixture(FIXTURE_CREATE).json),
    )
...
pub fn assert_prop_id() {
    ...
    assert!(
        wire_id.starts_with("report--"),
        "id must use report-- prefix: {wire_id}"
    );
...
macro_rules! t {
    ($req:literal, $name:ident, $assert:ident) => {
        interop_test!(
            $req,
            concat!("use_cases::report::producer::", stringify!($name)),
            $name,
            {
                $assert();
            }
