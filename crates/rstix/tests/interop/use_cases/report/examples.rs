...
pub fn assert_campaign_report() {
    ...
    assert!(fixture.provenance.divergence_recorded.iter().any(|d| d.defect == 16));
    ...
    assert!(bundle.get_typed::<Campaign>(&StixId::parse("campaign--83422c77-904c-4dc1-aff5-5c38f3a2c55c").unwrap()).is_some());
}
...
pub fn assert_malware_analysis_report() {
    ...
    assert!(bundle.get_typed::<Malware>(&StixId::parse("malware--bb4ca8dd-1248-5fef-8828-9bd2d935fa58").unwrap()).is_some());
}

interop_test!("REQ-3.16-EX-4.1", "use_cases::report::examples::campaign_report", campaign_report, { assert_campaign_report(); });
interop_test!("REQ-3.16-EX-4.2", "use_cases::report::examples::malware_analysis_report", malware_analysis_report, { assert_malware_analysis_report(); });
