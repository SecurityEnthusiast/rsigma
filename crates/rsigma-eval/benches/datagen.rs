...
/// Generate a single rule with `n_patterns` wildcard `|contains` values on
/// `CommandLine`. All patterns sit on the same field so the compiler emits
/// an `AnyOf` of `Regex` matchers, which the optimizer can batch into a
/// single `RegexSetMatch`.
///
/// Rules generated by this helper feed the regex-set throughput benchmark.
pub fn gen_regex_set_heavy_rule(rng: &mut StdRng, id: usize, n_patterns: usize) -> String {
    let mut values = String::new();
    for _ in 0..n_patterns {
        let pattern = WILDCARD_PATTERNS[rng.random_range(0..WILDCARD_PATTERNS.len())];
        values.push_str(&format!("            - '{pattern}'\n"));
    }
    format!(
        "title: Regex Set Heavy {id}\n\
         id: regex-set-heavy-{id:06}\n\
         logsource:\n\
         \x20   product: windows\n\
         \x20   category: process_creation\n\
         detection:\n\
...
/// Generate `n` regex-set-heavy rules, each with the same `n_patterns` count.
pub fn gen_n_regex_set_heavy_rules(n: usize, n_patterns: usize) -> String {
    let mut rng = rng();
    let mut docs = Vec::with_capacity(n);
    for i in 0..n {
        docs.push(gen_regex_set_heavy_rule(&mut rng, i, n_patterns));
    }
    docs.join("---\n")
}
