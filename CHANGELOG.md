...
**SIGHUP cert hot-reload.** The existing rules-reload signal also re-reads the certificate and key from disk and atomically swaps the active `rustls::ServerConfig` via `Arc<ArcSwap<…>>`. Inflight TLS connections are not dropped; failed reloads keep the previous certificate active and log an error so a typo in the path cannot black-hole the listener. Encrypted-key support and ACME/Let's Encrypt automation are intentionally out of scope; operators rotate cert files (cert-manager, certbot, Vault PKI, ...) and send `kill -HUP`.
