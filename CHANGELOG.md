...
- **`rsigma` CLI** — new `rsigma rule reverse --from <dialect>` (a single entry point for every dialect, mirroring `backend convert --target`) reads a query from an argument, `--file`, or stdin, takes `--title`/`--id`/`--level`/`--status` and `--logsource-*` hints a query cannot carry, and prints Sigma YAML (or writes it with `-o`). The emitted rule is parsed back before printing, so a rule that would not round-trip never reaches the operator.
