...
Recovery no longer panics. The stand-in path asserted that a hardcoded id still failed in the expected way, on a path reachable from parsing untrusted documents; malformed payloads now fall back to `ParseError::Json`. Reconstruction of `StixIdError::TypeMismatch` is also gone, along with the leaked allocation it needed to rebuild a `&'static str`, because `StixId::parse` cannot produce that variant and tagged recovery therefore never carried it. Typed-id conversion, which is where `TypeMismatch` originates, reports it through `serde::de::Error::custom` and surfaces as `ParseError::Json`.
