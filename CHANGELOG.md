...
### Dependency batch (Aug 2026)

Rolls up five open Dependabot PRs into a single merge. Rust (workspace `Cargo.lock`, with `ci/wasm-smoke/Cargo.lock` and `fuzz/Cargo.lock` synced for the `base64` bump): the patch group (#422) updates `pest`/`pest_derive` 2.8.7 to 2.8.8, `serde_json` 1.0.150 to 1.0.151, `jsonpath-rust` 1.0.5 to 1.0.6, `tokio-stream` 0.1.18 to 0.1.19, `time` 0.3.53 to 0.3.54, `jsonschema` 0.48.1 to 0.48.5, `clap` 4.6.2 to 4.6.4, and `rustls-pki-types` 1.15.0 to 1.15.1; standalone updates move `base64` 0.22.1 to 0.23.0 (#423), `toml` 0.8.23 to 1.1.3+spec-1.1.0 (#424), and `tikv-jemallocator` 0.6.1 to 0.7.0 (#425, jemalloc 5.3.1). CI (all repinned by commit SHA, batched via the `actions-updates` group, #421): `actions/checkout` v7.0.0 to v7.0.1, `taiki-e/install-action` v2.83.3 to v2.85.0, `docker/login-action` v4.4.0 to v4.5.0, `github/codeql-action/upload-sarif` v4.37.1 to v4.37.3, and `zizmorcore/zizmor-action` v0.6.0 to v0.6.1. Held back: `rusqlite` 0.40.1 (#234) requires Rust 1.89 while the workspace MSRV is Rust 1.88.
